...
const PeakSummary: React.FC = () => {
    ...
    const summary = summarizeResult(finalGains.clearSky);
    const maxTotalCS = summary.peak;
    const hourTotalCS_UTC = summary.hour;
    const hourTotalCS_Local = (hourTotalCS_UTC + offset) % 24;

    const sensibleAtPeak = summary.sensible;
    const latentAtPeak = summary.latent;
    ...
    const totalKWhCS = summary.coolingEnergyKWh;
        ...
        <Card className="flex-grow flex flex-col !p-0 overflow-hidden">
            ...
            <div className="p-6 flex-grow flex flex-col">
                <div className="flex flex-col items-center mb-8">
                    <p className="font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest text-xs mb-1 text-center">Maksymalne wymagane chłodzenie</p>
                ...
                <div className="text-sm text-slate-600 dark:text-slate-300 flex flex-col sm:flex-row gap-6 sm:gap-0 w-full mb-6">
                    ...
                    <div className="flex-1 flex flex-col items-center">
                        <p className="font-medium text-blue-500 uppercase tracking-wide text-xs mb-1">Utajone do pokrycia</p>
                ...
                <details className="text-xs mb-4"><summary>Bilans netto w godzinie szczytu</summary><p>Jawny: {(summary.sensibleNet / 1000).toFixed(2)} kW · Utajony: {(summary.latentNet / 1000).toFixed(2)} kW · Razem: {(summary.net / 1000).toFixed(2)} kW.</p>{summary.latentNet < 0 && <p>Ujemny bilans utajony oznacza usuwanie wilgoci przez wymianę powietrza. Nie pomniejsza wymaganej mocy jawnej.</p>}</details>
                 ...
                 {anyShadingEnabled && (
                    <div className="flex items-center justify-center gap-2 mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                        ...
                        <label className="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" aria-label="Wyniki z osłonami" checked={state.isShadingViewActive} onChange={(e) => dispatch({type: 'SET_SHADING_VIEW', payload: e.target.checked})} className="sr-only peer" />
                            <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-green-600"></div>
                        </label>
                        <Tooltip text="Przełącza wyniki całego projektu między wariantem z osłonami okien i bez osłon. Nie usuwa osłon z konfiguracji okien." position="top" />
                    </div>
                )}
            </div>
        </Card>
