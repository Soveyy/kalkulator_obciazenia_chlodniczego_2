...
const WallCard: React.FC<WallCardProps> = ({ wall }) => {
  ...
  const isUnconditioned = wall.boundaryType === 'unconditioned';
  const preset = CTS_PRESETS[wall.type];
  const isRoof = isRoofPreset(wall.type);
  const sameTypeWalls = walls.filter(w => isUnconditioned
    ? w.boundaryType === 'unconditioned'
    : w.boundaryType !== 'unconditioned' && isRoofPreset(w.type) === isRoof
  );
  const index = sameTypeWalls.findIndex(w => w.id === wall.id) + 1;
  const title = isUnconditioned
    ? `Przegroda do nieklimatyzowanej przestrzeni ${index}`
    : isRoof ? `Dach / stropodach ${index}` : `Ściana ${index}`;
  ...
  const handleEdit = () => dispatch({
    type: 'SET_MODAL',
    payload: { isOpen: true, type: isUnconditioned ? 'editUnconditionedWall' : 'editWall', data: wall.id }
  });
  ...
  const expectedModalType = isUnconditioned ? 'editUnconditionedWall' : 'editWall';
  const isEditing = state.modal.isOpen && state.modal.type === expectedModalType && state.modal.data === wall.id;

  const materialData = WALL_MATERIALS[wall.material || 'brick_red'];
  const tilt = wall.tilt ?? preset.defaultTilt;
  const unconditionedPreset = isUnconditioned
    ? UNCONDITIONED_PARTITION_PRESETS[wall.unconditionedType || 'ceiling_hot_attic']
    : null;
  ...
  return (
    <div className={`relative bg-slate-100 dark:bg-slate-700 p-4 rounded-lg shadow-sm flex flex-col min-h-48 transition-all duration-300 ${isNew ? 'ring-2 ring-green-400 ring-offset-2 ring-offset-slate-100 dark:ring-offset-slate-800' : ''} ${isEditing ? 'ring-2 ring-blue-500 ring-offset-2 ring-offset-slate-100 dark:ring-offset-slate-800' : ''}`}>
       {!isUnconditioned && tilt > 0 && <CompassArrow rotation={rotation} />}
      ...
      <div className="flex-grow text-sm space-y-1 text-slate-600 dark:text-slate-300">
        <p>Typ: <strong className="text-slate-800 dark:text-slate-100">{unconditionedPreset?.label || preset.label}</strong></p>
        {isUnconditioned ? (
          <>
            <p>Temperatura przestrzeni: <strong className="text-slate-800 dark:text-slate-100">{wall.adjacentTemperature ?? 50}°C</strong></p>
            <p>Metoda: <strong className="text-slate-800 dark:text-slate-100">stałe U × A × ΔT</strong></p>
          </>
        ) : (
          <>
        ...
        <p>{isUnconditioned ? unconditionedPreset?.areaLabel : isRoof ? 'Powierzchnia połaci' : 'Powierzchnia netto'}: <strong className="text-slate-800 dark:text-slate-100">{wall.area} m²</strong></p>
      </div>
